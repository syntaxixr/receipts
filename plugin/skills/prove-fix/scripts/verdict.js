...
export function isMissingSymbol(detail) {
    if (detail === undefined || !MISSING_SYMBOL.test(detail))
        return false;
...
export function decide(mode, green, red, redDetail) {
    if (green.length === 0 || green.every((o) => o === "missing")) {
        return { verdict: "SKIPPED", reason: "not collected by the test runner" };
    }
    if (green.every((o) => o === "skipped"))
        return { verdict: "SKIPPED", reason: "skipped with the change applied" };
    const gPass = green.includes("passed");
    const gFail = green.includes("failed");
    if (gPass && gFail)
        return { verdict: "FLAKY", reason: "passes and fails on the same code" };
    if (gFail)
        return { verdict: "BROKEN", reason: "fails with the change applied" };
    ...
    if (rPass && rFail)
        return { verdict: "FLAKY", reason: "passes and fails on the old code" };
    if (rPass) {
 ...
 */
export function relabelGuards(mode, verdicts) {
    if (mode === "refactor")
        return verdicts;
 ...
 */
export function firstErrorLine(detail) {
    if (!detail)
        return undefined;
    const lines = detail.split("\n").map((l) => l.trim()).filter(Boolean);
    if (lines[0]?.startsWith("timed out"))
        return lines[0];
