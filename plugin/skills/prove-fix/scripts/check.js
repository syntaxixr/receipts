...
import { changedFiles, changedLines, commitSubjects, gitDir, hasTrackedChanges, headSha, isDirty, mergeBase, repoRoot, resolveBase, showAtRev, } from "./git.js";
import { expandChangedTests } from "./locate/expand.js";
import { findShadowedSources } from "./shadow.js";
import { detectMode } from "./mode.js";
import { availableRunners, lookup } from "./runners/index.js";
...
function runAndRecord(root, tracks, side, opts) {
    const imported = new Set();
    const byRunner = new Map();
    for (const t of tracks)
        byRunner.set(t.runner, [...(byRunner.get(t.runner) ?? []), t]);
    for (const [runner, group] of byRunner) {
        ...
        for (const tr of group) {
            ...
            if (r.outcome === "failed" && r.detail) {
                if (side === "red")
                    tr.redDetail ??= r.detail;
                else
                    tr.greenDetail ??= r.detail;
                if (run.timedOut && r.detail.startsWith(run.timedOut))
                    tr.hung = true;
            }
        }
    }
...
function rerunFailures(root, tracks, side, opts) {
...
export function runCheck(opts) {
    ...
    const changes = changedFiles(root, mb, opts.committed).map((c) => ({ ...c, kind: classify(c.path) }));
    ...
    for (const f of testChanges) {
        ...
        if (!runner) {
            ...
            if (/\.(test|spec)\.[cm]?[jt]sx?$/.test(f.path) && f.status !== "deleted")
                unsupported.push(f.path);
            continue;
        }
        ...
        for (const test of changed)
            tracks.push({ test, runner, green: [], red: [] });
        if (changed.length > 0)
    ...
    if (changes.length === 0 || (sources.length === 0 && tracks.length === 0))
        receipt.status = "no-changes";
    else if (tracks.length === 0)
        receipt.status = "no-tests";
    else if (sources.length === 0)
        receipt.status = "test-only";
    if (receipt.status === "checked") {
        ...
        if (shadowed.length > 0) {
            receipt.warnings.push(`The tests do not run this checkout's code: ${shadowed.join("; ")}. ` +
        ...
        tracks.forEach((tr, i) => {
            ...
            const reason = verdict === decided[i].verdict
                ? decided[i].reason
                : verdict === "GUARD"
                    ? GUARD_REASON
                    : "not judged: the tests import the code from outside this checkout (see warning)";
            const raw = verdict === "BROKEN" || verdict === "FLAKY"
    ...
    for (const [verdict, count] of Object.entries(receipt.summary)) {
        if (failOn.has(verdict.toLowerCase()))
            receipt.violations.push(`${count} ${verdict}`);
    }
    if (receipt.status === "no-tests" && failOn.has("no-tests"))
        receipt.violations.push("code changed without tests");
