 ...
 */
export function locatePythonTests(file, source) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!def[2].startsWith("test"))
            continue;
        if (owner && (owner.kind !== "class" || !owner.isTestClass))
            continue;
        let end = headerEnd;
...
export function locatePythonSuites(source) {
    const lines = source.split(/\r?\n/);
    const suites = [];
    for (let i = 0; i < lines.length; i++) {
        const cls = CLASS_RE.exec(lines[i]);
        if (!cls || !(cls[2].startsWith("Test") || /Test/.test(cls[3] ?? "")))
            continue;
