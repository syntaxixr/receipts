...
function pickPython(explicit) {
    if (explicit)
        return explicit;
    if (process.env.RECEIPTS_PYTHON)
        return process.env.RECEIPTS_PYTHON;
 ...
 */
export function runPytest(root, testFiles, opts = {}, tests = []) {
    if (tests.length > 0) {
    ...
    if (!byFile)
        throw new Error(`pytest did not produce results for ${testFiles.join(", ")}`);
    return byFile;
}
function runPytestArgs(root, targetsIn, opts) {
    ...
    try {
        ...
        if (proc.error && !timedOut)
            throw proc.error;
        const records = fs.existsSync(outFile)
        ...
        for (const rec of records) {
            ...
            if (rec.type === "collectError") {
                if (rec.path)
                    fileErrors.set(relPath(root, rec.path), rec.detail);
                continue;
            }
            if (rec.type !== "result")
                continue;
            ...
            if (rec.when === "call" || rec.outcome !== "passed")
                item.finished = true;
