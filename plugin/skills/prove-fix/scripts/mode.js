 ...
 */
export function detectMode(explicit, subjects) {
    if (explicit) {
        if (!isMode(explicit))
            throw new Error(`--mode must be fix, feat or refactor (got "${explicit}")`);
        return { mode: explicit, source: "--mode" };
    }
    const env = process.env.RECEIPTS_MODE;
    if (env && isMode(env))
        return { mode: env, source: "RECEIPTS_MODE" };
        ...
        try {
            ...
            if (pr) {
                for (const label of pr.labels ?? []) {
                    ...
                    if (hit)
                        return { mode: hit[1], source: `PR label "${label.name}"` };
                }
                const byTitle = fromText(String(pr.title ?? ""));
                if (byTitle)
                    return { mode: byTitle, source: "PR title" };
            }
        }
        catch {
    ...
    if (modes.includes("fix"))
        return { mode: "fix", source: "commit messages" };
    if (modes.includes("feat"))
        return { mode: "feat", source: "commit messages" };
    if (modes.length > 0 && modes.every((m) => m === "refactor"))
        return { mode: "refactor", source: "commit messages" };
