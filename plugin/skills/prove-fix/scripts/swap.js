...
function removeEntry(abs) {
    const st = lstatOrNull(abs);
    if (!st)
        return;
    if (st.isDirectory())
        throw new UnsafePathError(`refusing to remove directory "${abs}"`);
    fs.unlinkSync(abs);
}
function baseTarget(root, mb, rel) {
...
export function journalDir(gitDirPath) {
    return path.join(gitDirPath, "receipts-backup");
}
export function hasPendingJournal(gitDirPath) {
    return fs.existsSync(path.join(journalDir(gitDirPath), "journal.json"));
}
function ensureDir(root, dir, created) {
    const missing = [];
    let cur = dir;
...
export function revertSources(rootIn, gitDirPath, mb, sources) {
    ...
    const journal = { createdDirs: [], entries: [] };
    ...
    for (const [rel, target] of targets) {
        ...
        writeJournal();
        if (target.kind === "link")
            fs.symlinkSync(target.target, abs);
        else
 ...
 */
export function restoreSources(rootIn, gitDirPath) {
    const dir = journalDir(gitDirPath);
    ...
    for (const e of journal.entries) {
        try {
            ...
            if (e.link !== undefined) {
                ...
                if (e.mode !== undefined)
                    fs.chmodSync(abs, e.mode & 0o7777);
            }
        }
        catch (err) {
    ...
    if (failures.length > 0) {
        throw new Error(`could not restore ${failures.length} file(s); the originals are kept in ${dir}:\n  ${failures.join("\n  ")}`);
    }
