...
const ORDER = ["BROKEN", "THEATER", "CHANGED", "FLAKY", "WEAK", "SKIPPED", "PROVEN", "PRESERVED", "GUARD"];
function sorted(r) {
...
export function toText(r, color) {
    ...
    if (r.tests.length > 0) {
        ...
        for (const t of sorted(r)) {
            ...
            if (t.detail && t.verdict !== "PROVEN")
                out.push(`  ${" ".repeat(12 + width)}${c("2", `↳ ${plain(t.detail)}`)}`);
        }
        out.push("");
    }
    ...
    if (r.unsupportedTestFiles.length > 0) {
        out.push(c("33", `  No runner for: ${r.unsupportedTestFiles.map((t) => plain(t, 160)).join(", ")} (add vitest or jest to package.json, or pass --js-runner)`));
        out.push("");
    }
    for (const w of r.warnings)
        out.push(c("33", `  Warning: ${plain(w, 600)}`), "");
...
export function toMarkdown(r) {
    ...
    out.push(statusLine(r));
    out.push("");
    out.push("<sub>Each changed test runs twice: with the change, and with the code under test reverted to the base. " +
