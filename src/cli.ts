...
function main(argv: string[]): number {
  ...
  if (!Number.isInteger(reruns) || reruns < 0) throw new Error("--reruns must be a non-negative integer");
  ...
  if (values.json) fs.writeFileSync(values.json, toJson(receipt) + "\n");
  const md = toMarkdown(receipt);
  if (values.markdown) fs.writeFileSync(values.markdown, md + "\n");
