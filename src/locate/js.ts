...
class Scanner {
  ...
  skipTemplate(i: number): number {
    i++;
    while (i < this.src.length && this.src[i] !== "`") {
      if (this.src[i] === "\\") i += 2;
      else if (this.src[i] === "$" && this.src[i + 1] === "{") i = this.skipBalanced(i + 1, "{", "}");
      else i++;
    }
    return i + 1;
  }

  skipRegex(i: number): number {
    i++;
    let inClass = false;
  ...
  skipTrivia(i: number): number {
    ...
    if (ch === '"' || ch === "'") return this.skipString(i);
    if (ch === "`") return this.skipTemplate(i);
    if (ch === "/" && this.regexAllowed(i)) return this.skipRegex(i);
    return i;
  }

  regexAllowed(i: number): boolean {
    let j = i - 1;
    while (j >= 0 && /\s/.test(this.src[j]!)) j--;
    if (j < 0) return true;
  ...
  skipBalanced(i: number, open: string, close: string): number {
    let depth = 0;
    while (i < this.src.length) {
...
/** A describe/suite block's line range, 1-based and inclusive. */
export interface SuiteRange {
  startLine: number;
  endLine: number;
}

function scan(source: string): { blocks: Block[]; lineStarts: number[] } {
  ...
  while (i < source.length) {
    ...
    for (;;) {
      ...
      if (CALLED_MODIFIERS.has(mod[0])) {
        ...
        if (source[k] === "(") j = sc.skipBalanced(k, "(", ")");
        else if (source[k] === "`") j = sc.skipTemplate(k);
        else ok = false;
      }
    }
    j = sc.skipSpace(j);
    if (!ok || source[j] !== "(") continue;
...
export function locateJsSuites(source: string): SuiteRange[] {
...
export function locateJsTests(file: string, source: string): TestCase[] {
  const { blocks, lineStarts } = scan(source);
  ...
  for (const b of blocks) {
    ...
    if (blocks.some((o) => o !== b && o.kind === "test" && o.start < b.start && o.end >= b.end)) continue;
    const scope = suites
