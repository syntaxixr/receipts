 ...
 */
export function findShadowedSources(root: string, sources: string[], imported: Set<string>): string[] {
  ...
  for (const rel of sources) {
    ...
    const twin = outside.find((f) => {
      ...
      if (modulePath.includes("/")) return norm.endsWith(`/${modulePath}`);
    ...
    if (twin) notes.push(`${rel} was imported from ${twin}, not from this checkout`);
  }
  return notes;
}
